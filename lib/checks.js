...
// ── 3b) install-time protection: Aikido Safe Chain ──────────────────────────
// Every check in this file is RETROSPECTIVE: it judges a lockfile that
// already resolved, after `install` already ran. Aikido Safe Chain is the
// complementary REAL-TIME layer — it wraps npm/yarn/pnpm/bun/pip/poetry so a
// malicious release is blocked the moment it would install, before its
// preinstall/postinstall script gets to run. That is the exact mechanism the
// Aug 2026 keyv/Shai-Hulud worm used to drop its payload (a `preinstall`
// entry added to package.json), and it is a blind spot none of the checks
// above can close — they only ever see a lockfile after the fact.
// We can only check whether the binary is on PATH from inside a subprocess;
// confirming it is wired into the user's interactive shell needs
// `{pm} safe-chain-verify` run by hand, so we word the OK line accordingly.
export function installTimeProtection(d) {
  const title = 'install-time protection — Aikido Safe Chain';
  if (!d.npm && !d.python) return { title, lines: [{ status: 'skip', text: 'no npm or PyPI ecosystem detected' }] };
  if (has('safe-chain')) {
    return { title, lines: [{ status: 'ok', text: 'safe-chain is on PATH — run `npm safe-chain-verify` if you have not confirmed it is active in this shell' }] };
  }
  return {
    title,
    lines: [
      { status: 'info', text: 'not installed — this scan only judges a lockfile that already resolved; safe-chain blocks a malicious install in real time, before any preinstall/postinstall script runs' },
      { status: 'info', text: 'install once: curl -fsSL https://github.com/AikidoSec/safe-chain/releases/latest/download/install-safe-chain.sh | sh' },
    ],
  };
}
